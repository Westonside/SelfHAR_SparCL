...
for itera in range(nb_groups): # for each task
  ...
  if itera == 0: # if the first task
    files_from_cl = files_train[itera]
  else:
    files_from_cl = files_train[itera][:]
    for i in range(itera*nb_cl):
      nb_protos_cl  = int(np.ceil(nb_proto*nb_groups*1./itera)) # Reducing number of exemplars of the previous classes number of exemplars per class calculated by num task * classes / task
  ...
  if itera == 0:
    # No distillation, meaning that you are not storing exemplars in the buffer
    variables_graph,variables_graph2,scores,scores_stored = utils_icarl.prepare_networks(gpu,image_batch, nb_cl, nb_groups) # this will create a variable scope which allows storing vars and a new model
  ...
  if itera > 0: #if not the first task
    # Distillation
